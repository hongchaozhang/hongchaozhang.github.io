...
CPU、缓存和内存的关系：
![cpu和内存的关系类比.png](/images/cpu和内存的关系类比.png)
...
为了提高SSD的使用寿命，我们要的就是想一个办法，让SSD硬盘各个块的擦除次数，均匀分摊到各个块上。这个策略呢，就叫作**磨损均衡**（Wear-Leveling）。实现这个技术的核心办法，和我们前面讲过的虚拟内存一样，就是添加一个间接层，就是FTL这个**闪存转换层**。

![闪存转换层.jpeg](/images/闪存转换层.jpeg)
