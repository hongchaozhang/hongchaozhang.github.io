...
> Similar question: 给定一个存放整数的数组，重新排列数组使得数组左边为奇数，右边为偶数。时间复杂度O(n),空间复杂度O(1)。


### <a name="6"></a>Building Bridge

A and B live in different sides of a straight river, whose width is `w`. A and B want to build a bridge over the river, so that they can visit each other. **Question:** where to build the bridge so that the walking distance is the shortest between A and B?
**Note:** The bridge must be perpendicular (vertical) to the river.
...
[关于选牛和选马的问题](http://blog.csdn.net/tianmohust/article/details/6839524)
...
* 将1000吨煤运送到目的地，此时还剩1000{1-[1/(2n-1)+1/(2n-3)+...+1/3]}吨煤。
...
所以，当n=8时，可以将多于1000吨的煤运送到目的地。火车可以将2000吨煤运送到1000[1/5+1/7+...+1/13+1/15]公里处，即688公里处，最终能将2000-(1000-688)*3吨煤，即1065吨煤，运送到目的地。
